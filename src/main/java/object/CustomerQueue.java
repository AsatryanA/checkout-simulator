...
public class CustomerQueue extends HBox {
    ...
    public CustomerQueue() {
        ...
        if (trolley == null) {
            trolley = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/image/trolley.png")));
        }
    ...
    public void initTooltipService(long period) {
        ...
        tooltipUpdateTask = MainModel.getInstance().getThreadPoolExecutor().scheduleAtFixedRate(() -> Platform.runLater(() -> tooltip.setText("հաճախորդ " + nowCustomer.getNo() + "\n\n" +
                "ապրանքների քանակը: " + nowCustomer.getQuantityOfGoods() + "\n" +
                "սպասվում է սպասարկելու: " + nowCustomer.getQuantityWaitForCheckout() + "\n" +
                "սպասման ժամանակը: " + nowCustomer.getWaitSecActual() + "վ\n" +
                "բնավորություն: " + (nowCustomer.isCannotWait() ? "վատ  :(" : "լավ :)") +
                (nowCustomer.isCannotWait() ? "\nհետո կհեռանա:" + (nowCustomer.getWaitSec() - nowCustomer.getWaitSecActual()) + "վ" : ""))), 0, period, TimeUnit.MILLISECONDS);
    }
